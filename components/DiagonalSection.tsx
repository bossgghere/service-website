...
const DiagonalSection: React.FC<Props> = ({ 
  ...
  return (
    <section className="group relative py-24 sm:py-32 lg:py-48 overflow-hidden z-20 flex items-center min-h-[600px] border-y border-neutral-100 bg-black">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-8 sm:px-12 w-full">
        <div className={`max-w-2xl ${isDark ? 'text-white' : 'text-black'}`}>
          <h2 className="text-4xl sm:text-6xl lg:text-7xl font-black uppercase mb-10 leading-[0.9] tracking-tighter">
            {title}
          </h2>
          ...
          {ctaText && (
            <button className={`group/btn relative px-10 py-5 text-[10px] sm:text-xs font-black tracking-[0.4em] uppercase transition-all overflow-hidden border ${isDark ? 'border-white/20 text-white' : 'border-black/20 text-black'}`}>
